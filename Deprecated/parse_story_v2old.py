...
def move_to_previous_selection(selection):
	global current_label
	global current_selection

	option = list(selection.items())[-1]
	responses = option[1]['responses']
	if len(responses) > 0 and responses[-1]['selection'] != {}:
		move_to_previous_selection(responses[-1].selection)
		return
	current_label = option[0]
				...
				for action in parsed_text:
					try:
						...
						if action['command_name'] == 'end_sel':
							...
							if current_selection == selection_hierarchy:
								print_selection(selection_hierarchy, 0)
								current_selection = {}
								selection_hierarchy = {}
								current_label = None
							else:
								move_to_previous_selection(selection_hierarchy)
